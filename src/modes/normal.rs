...
const CTRL_V: char = '\x16'; // Control-V
	...
	pub fn try_parse(&mut self, ch: char) -> Option<ViCmd> {
		...
		let verb = 'verb_parse: {
			...
			match ch {
				...
				CTRL_V => { // icky but it works
					return Some(
						ViCmd {
							register,
							verb: Some(VerbCmd(count, Verb::VisualModeBlock)),
							motion: None,
							raw_seq: self.take_cmd(),
							flags: self.flags()
						}
...
impl ViMode for ViNormal {
	fn handle_key(&mut self, key: E) -> Option<ViCmd> {
		let mut cmd = match key {
			...
			E(K::Char('V'), M::CTRL) => {
				// FIXME: this is a hack if i've ever seen one
				self.try_parse(CTRL_V)
			}
